...
Added secure-room E2EE foundation per ADR-0011. Client-side AES-256-GCM per-Yjs-update encryption with PKBDF2 key derivation from 12-word BIP39 recovery phrases.

@eweser/shared: New types — `RoomEncryptionMetadata`, `RoomCryptoStatus`, `RoomKeyExportMaterial`, `WrappedRoomKey`, `KeyExportFormat`. `ServerRoom` gains optional `encryption` field.
