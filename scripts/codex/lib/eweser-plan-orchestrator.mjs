...
const CODING_AGENTS = new Set(['eweser-coder']);
const MANUAL_TEST_AGENT = 'eweser-manual-tester';
const BROWSER_CHECKPOINT_AGENT = 'eweser-browser-checkpoint';
const SUPPORTED_RUN_AGENTS = new Set([
  ...CODING_AGENTS,
  MANUAL_TEST_AGENT,
  BROWSER_CHECKPOINT_AGENT,
]);
const SUPPORTED_BROWSER_CHECKPOINTS = new Set(['none', 'focused', 'full']);
const LIKELY_UI_SCOPE_PATTERNS = [
  /^packages\/app\/src\//,
  /^packages\/ewe-note\/src\//,
  /^packages\/examples-components\/src\//,
  /^examples\/[^/]+\/src\//,
  /\.(css|scss|sass|less|tsx|jsx|html)$/i,
];
...
function selectRuns(plan, runId) {
  const runs = expandRuns(normalizeRuns(plan.metadata.runs ?? []));
  const selectedRuns = runId ? selectRequestedRuns(runs, runId) : runs;
...
function selectRequestedRuns(runs, runId) {
  const directRun = runs.find((run) => run.id === runId);
  if (!directRun) return [];
  const selected = runs.filter(
    (run) =>
      run.id === runId ||
      run.checkpointTarget === runId ||
      (directRun.autoGenerated && run.id === directRun.checkpointTarget)
  );
  return selected.map((run) => {
    if (run.autoGenerated) return run;
...
function normalizeRuns(runs) {
  ...
  return runs.map((run) => ({
    ...
    autoGenerated: false,
    browserCheckpoint: normalizeBrowserCheckpoint(run.browserCheckpoint),
    ...
    title: run.title,
    ui: typeof run.ui === 'boolean' ? run.ui : undefined,
...
function normalizeBrowserCheckpoint(value) {
  if (value === undefined || value === null || value === '') return undefined;
  if (value === true) return 'focused';
...
function expandRuns(runs) {
  ...
  for (const run of runs) {
    expanded.push(run);
    if (!CODING_AGENTS.has(run.agent)) continue;
    const checkpointMode = resolvedBrowserCheckpoint(run);
    if (!checkpointMode || checkpointMode === 'none') continue;
    expanded.push({
      agent:
        checkpointMode === 'full'
          ? MANUAL_TEST_AGENT
          : BROWSER_CHECKPOINT_AGENT,
      allowSharedScope: false,
      autoGenerated: true,
      browserCheckpoint: checkpointMode,
      changeset: 'no',
      checkpointTarget: run.id,
      dependsOn: [run.id],
      id: `${run.id}--${checkpointMode === 'full' ? 'manual-test' : 'browser-checkpoint'}`,
      model: 'strong',
      parallel: false,
      sourceRunId: run.id,
      tests: [],
      title:
        checkpointMode === 'full'
          ? `Manual Test Gate: ${run.title}`
          : `Focused Browser Checkpoint: ${run.title}`,
      ui: false,
      writeScope: [],
    });
  }
...
function resolvedBrowserCheckpoint(run) {
  if (!CODING_AGENTS.has(run.agent)) return run.browserCheckpoint;
  if (run.browserCheckpoint) return run.browserCheckpoint;
  if (run.ui === true) return 'focused';
  return undefined;
}
...
function validatePlan(plan) {
  ...
  for (const run of plan.allRuns) {
    ...
    if (!SUPPORTED_RUN_AGENTS.has(run.agent))
      throw new Error(`Run ${run.id} uses unsupported agent: ${run.agent}`);
    if (
      run.browserCheckpoint !== undefined &&
      !SUPPORTED_BROWSER_CHECKPOINTS.has(run.browserCheckpoint)
    ) {
      throw new Error(
        `Run ${run.id} uses unsupported browserCheckpoint: ${run.browserCheckpoint}`
      );
    }
    if (!['coding', 'strong', 'simple', 'fast', 'mini'].includes(run.model))
      throw new Error(`Run ${run.id} uses unsupported model: ${run.model}`);
    if (CODING_AGENTS.has(run.agent) && run.writeScope.length === 0)
      throw new Error(`Run ${run.id} must declare writeScope`);
    if (
      CODING_AGENTS.has(run.agent) &&
      run.ui === undefined &&
      touchesLikelyUiScope(run.writeScope)
    ) {
      throw new Error(
        `Run ${run.id} touches likely UI scope. Declare ui: true to require a browser checkpoint, or ui: false to mark it as intentionally non-UI.`
      );
    }
...
function touchesLikelyUiScope(scopes) {
  return scopes.some((scope) =>
    LIKELY_UI_SCOPE_PATTERNS.some((pattern) =>
      pattern.test(normalizeScope(scope))
    )
  );
}
...
function workerPrompt(plan, run) {
  if (CODING_AGENTS.has(run.agent)) {
    return `Implement run ${run.id} from ${plan.relativePath} using $eweser-coder.
...
Finish with: files changed, verification commands and outcomes, blockers, and residual risks.`;
  ...
  if (run.agent === MANUAL_TEST_AGENT) {
    return `Run ${run.id} from ${plan.relativePath} using $eweser-manual-tester.

Do not implement fixes. Your job is independent verification only.
This run is${run.autoGenerated ? ' an orchestrator-generated UI gate' : ''} for: ${run.sourceRunId}
Run title: ${run.title}
Focus on the user-visible behavior delivered by run ${run.sourceRunId}. Use the plan handoff, visible local app surfaces, browser evidence, and any listed verification commands as the source of truth.
If Browser Use IAB is unavailable, record that exact blocker and fall back as the skill instructs.
For CLI worker agents, Playwright CLI is an accepted primary browser path. Only mention Browser Use failure details if you actually attempted IAB first.
...
Finish with: result (pass | pass with notes | fail | blocked), findings ordered by severity, exact reproduction steps, evidence, coverage, and residual risk.`;
  }

  return `Run ${run.id} from ${plan.relativePath} using $eweser-browser-checkpoint.

Do not implement fixes. This is a focused browser checkpoint, not a full exploratory manual test.
Checkpoint target: ${run.sourceRunId}
Run title: ${run.title}
Verify only the key user-visible behavior changed by run ${run.sourceRunId}. Prefer 1 to 3 critical flows or surfaces, use the Codex in-app browser first, and keep the report concise and evidence-backed.
For CLI worker agents, Playwright CLI is an accepted primary browser path. If Browser Use IAB is unavailable after an attempted IAB start, record that exact blocker and use the documented Playwright path. Do not expand into a broad QA sweep.
...
Finish with: result (pass | pass with notes | fail | blocked), focused findings, exact evidence, and whether a full manual test gate is still recommended.`;
}
