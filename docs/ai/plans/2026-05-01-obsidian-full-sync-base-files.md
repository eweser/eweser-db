...
- **Steps**:
  - [x] Implement the approved slice of `docs/ai/plans/2026-04-03-file-storage.md` needed for attachment bytes: S3-compatible auth-server storage adapter, upload/download/presign routes, and DB/EweNote helpers.
  - [x] Validate access-grant-token and session-based room access for file upload/download routes.
  - [x] Keep provider credentials out of room data and ordinary manifests; store only provider profile ids, object keys, hashes, and non-secret metadata in synced documents.
  - [x] Store object keys and hashes in attachment metadata documents.
  - [x] Materialize remotely stored files back into the mounted Obsidian vault when a device mounts that base.
  - [x] Enforce a configurable per-file size limit in the auth-server upload route.
  - [ ] Add documented provider-profile UX for hosted Eweser storage, self-hosted MinIO, and bring-your-own S3-compatible providers beyond the initial Railway Buckets mapping.
  - [ ] Add a durable local binary cache/pinning layer beyond vault materialization.
  - [ ] Document the production/self-hosted env mapping and operator setup path.
- **Tests**:
...
- **Verification**:
  - `npm run check`
  - Mocked object storage integration smoke test.
- **Manual test handoff**:
...
| `run-5`  | Completed   | `packages/auth-server-hono/src/lib/storage.ts`, `packages/auth-server-hono/src/routes/files.ts`, `packages/auth-server-hono/src/routes/files.test.ts`, `packages/auth-server-hono/src/index.ts`, `packages/auth-server-hono/src/index.test.ts`, `packages/auth-server-hono/src/env.ts`, `packages/auth-server-hono/package.json`, `package-lock.json`, `packages/db/src/utils/files.ts`, `packages/db/src/utils/files.test.ts`, `packages/db/src/utils/index.ts`, `packages/db/src/index.ts`, `packages/ewe-note/src/cli/vault-sync.ts`, `docs/ai/plans/2026-05-01-obsidian-full-sync-base-files.md` | `npm test --workspace @eweser/auth-server-hono`, `npm test --workspace @eweser/db`, `npm test --workspace @eweser/ewe-note -- vault-sync`, `npm run type-check --workspace @eweser/auth-server-hono`, `npm run type-check --workspace @eweser/db`, `npm run type-check --workspace @eweser/ewe-note`, `npm run build --workspace @eweser/db`, `npm run build --workspace @eweser/ewe-note`, `npm run lint --workspace @eweser/auth-server-hono`, `npm run lint --workspace @eweser/db`, `npm run lint --workspace @eweser/ewe-note -- --max-warnings=0` | Implements a provider-agnostic S3-compatible storage adapter with Railway Buckets as the first hosted target, adds guarded file upload/presign/download routes, adds SDK upload/download helpers, and updates vault-sync so remote attachment records can upload bytes and materialize them back to disk on another mount. Live Railway credential smoke was not run in this turn. |
| `run-6`  | Not started |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |                                                                                                                                                                                                                                                                                                                                                                                    |
...
- Railway Buckets first was the right initial hosted provider choice because it keeps the server/client contract S3-compatible without carrying the extra service/volume overhead of MinIO-on-Railway on day one.
