...
| `run-7` | Local complete, production in review | Tasks `t_fefab561` (done), `t_ee65ba6f` (pr_review); PRs #47 (merged), #56 (open)                         | Local pass: landing build passes, app type-check passes, pre-push checks clean. Production pass: landing build passes, app type-check passes, Copilot reviewed, mergeable                                                                                                                                     | Local site-promise reconciliation PR #47 merged. Production-evidence pass `t_ee65ba6f` has PR #56 open: updates landing/connected-tools/app copy from "local" to "production-verified" based on run-5 MCP smoke evidence (3 files changed). PR #56 is mergeable and awaiting human review. Remaining pending: OAuth client validation, non-Claude-Desktop client proofs.                                                                                                                                                                                                                                                                           |
| `run-8` | Deferred                             | Task `t_41ed0379`; 32 child backlog cards                                                                 | Board DB integrity verified ok; child backlog cards created with micro-example + Ewe Note e2e acceptance paths; `PRAGMA integrity_check` = ok                                                                                                                                                                 | Read-only DB README backlog split deferred until run-7 completes. 32 child cards in `todo` covering federation, secure rooms, compatibility, SDK lifecycle, agent access, cross-collection queries, and usage metering. Board DB recovered from two malformed-write incidents (May 25) with integrity now verified.                                                                                                                                                                                                                                                                                                                                |
