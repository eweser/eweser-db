...
const messageReconnectTimeout$1 = 3e4;
const setupWS$1 = (wsclient) => {
  if (wsclient.shouldConnect && wsclient.ws === null) {
    ...
    websocket.onmessage = (event) => {
      wsclient.lastMessageReceived = getUnixTime$1();
    ...
    const onclose = (error) => {
      if (wsclient.ws !== null) {
        ...
        setTimeout(setupWS$1, min$1(log10(wsclient.unsuccessfulReconnects + 1) * reconnectTimeoutBase, maxReconnectTimeout), wsclient);
      }
      clearTimeout(pingTimeout);
    };
    const sendPing = () => {
...
class WebsocketClient extends Observable$1 {
  ...
  constructor(url, { binaryType } = {}) {
    ...
    this._checkInterval = setInterval(() => {
      if (this.connected && messageReconnectTimeout$1 < getUnixTime$1() - this.lastMessageReceived) {
        this.ws.close();
      }
...
const onChange$1 = (eventHandler) => usePolyfill$1 || addEventListener(
  "storage",
  /** @type {any} */
  eventHandler
);
const offChange$1 = (eventHandler) => usePolyfill$1 || removeEventListener(
  "storage",
  /** @type {any} */
  eventHandler
);
...
let nextColor = 0;
let lastLoggingTime = getUnixTime$1();
const createModuleLogger$1 = (_print, moduleName) => {
  const color = loggingColors[nextColor];
  const debugRegexVar = getVariable$1("log");
...
const fromBase64Node$1 = (s) => {
  ...
  return createUint8ArrayViewFromArrayBuffer$1(buf.buffer, buf.byteOffset, buf.byteLength);
};
const toBase64$1 = isBrowser$1 ? toBase64Browser$1 : toBase64Node$1;
const fromBase64$1 = isBrowser$1 ? fromBase64Browser$1 : fromBase64Node$1;
const copyUint8Array = (uint8Array) => {
  const newBuf = createUint8ArrayFromLen$1(uint8Array.byteLength);
  newBuf.set(uint8Array);
  return newBuf;
};
const channels$1 = /* @__PURE__ */ new Map();
let LocalStoragePolyfill$1 = class LocalStoragePolyfill {
  /**
   * @param {string} room
   */
  constructor(room) {
    ...
    onChange$1(this._onChange);
  }
  /**
   * @param {ArrayBuffer} buf
   */
  postMessage(buf) {
    varStorage$1.setItem(this.room, toBase64$1(createUint8ArrayFromArrayBuffer$1(buf)));
  }
  close() {
    offChange$1(this._onChange);
  }
};
const BC$1 = typeof BroadcastChannel === "undefined" ? LocalStoragePolyfill$1 : BroadcastChannel;
const getChannel$1 = (room) => setIfUndefined$1(channels$1, room, () => {
  const subs = create$5();
  const bc = new BC$1(room);
...
const subscribe$1 = (room, f) => {
  getChannel$1(room).subs.add(f);
  return f;
};
const unsubscribe$1 = (room, f) => {
  const channel = getChannel$1(room);
  const unsubscribed = channel.subs.delete(f);
  if (unsubscribed && channel.subs.size === 0) {
    channel.bc.close();
    channels$1.delete(room);
  }
  return unsubscribed;
};
const publish$1 = (room, data, origin = null) => {
  const c = getChannel$1(room);
...
class Doc extends ObservableV2 {
  ...
  constructor({ guid = uuidv4(), collectionid = null, gc = true, gcFilter = () => true, meta = null, autoLoad = false, shouldLoad = true } = {}) {
    ...
    this.isLoaded = false;
    this.isSynced = false;
    this.whenLoaded = create$1((resolve2) => {
      this.on("load", () => {
        this.isLoaded = true;
        resolve2(this);
      });
    });
    const provideSyncedPromise = () => create$1((resolve2) => {
  ...
  (function(e) {
    ...
    return (/* @__PURE__ */ function() {
      function d(s, e, n2) {
        ...
        for (var r2 = "function" == typeof commonjsRequire && commonjsRequire, a = 0; a < n2.length; a++)
          t2(n2[a]);
        return t2;
      }
      return d;
      ...
      (function() {
        (function() {
          ...
          n2.kMaxLength = 2147483647, s.TYPED_ARRAY_SUPPORT = function() {
            ...
            return R2(this, e2, t4, n3, true);
          }, s.prototype.lastIndexOf = function(e2, t4, n3) {
            return R2(this, e2, t4, n3, false);
      ...
      o.read = function(t2, n2, a2, o2, l) {
        ...
        var h, b, y, g = Math.LN2, _ = Math.log, C = 8 * f - p - 1, R2 = (1 << C) - 1, E = R2 >> 1, w = 23 === p ? r(2, -24) - r(2, -77) : 0, S = u ? 0 : f - 1, T = u ? 1 : -1, d = 0 > o2 || 0 === o2 && 0 > 1 / o2 ? 1 : 0;
        for (o2 = n(o2), isNaN(o2) || o2 === 1 / 0 ? (b = isNaN(o2) ? 1 : 0, h = R2) : (h = t(_(o2) / g), 1 > o2 * (y = r(2, -h)) && (h--, y *= 2), o2 += 1 <= h + E ? w / y : w * r(2, 1 - E), 2 <= o2 * y && (h++, y /= 2), h + E >= R2 ? (b = 0, h = R2) : 1 <= h + E ? (b = (o2 * y - 1) * r(2, p), h += E) : (b = o2 * r(2, E - 1) * r(2, p), h = 0)); 8 <= p; a2[l + S] = 255 & b, S += T, b /= 256, p -= 8)
          ;
...
const deriveKey = (secret, roomName) => {
  const secretBuffer = encodeUtf8$1(secret).buffer;
  const salt = encodeUtf8$1(roomName).buffer;
...
const encrypt = (data, key) => {
  ...
  return crypto.subtle.encrypt(
    ...
    const encryptedDataEncoder = createEncoder$1();
    writeVarString(encryptedDataEncoder, "AES-GCM");
    writeVarUint8Array$1(encryptedDataEncoder, iv);
    writeVarUint8Array$1(encryptedDataEncoder, new Uint8Array(cipher));
    return toUint8Array$1(encryptedDataEncoder);
  });
};
const encryptJson = (data, key) => {
  const dataEncoder = createEncoder$1();
  writeAny(dataEncoder, data);
  return encrypt(toUint8Array$1(dataEncoder), key);
};
const decrypt = (data, key) => {
  ...
  const dataDecoder = createDecoder$1(data);
  const algorithm = readVarString(dataDecoder);
  if (algorithm !== "AES-GCM") {
    reject(create$4("Unknown encryption algorithm"));
  }
  const iv = readVarUint8Array$1(dataDecoder);
  const cipher = readVarUint8Array$1(dataDecoder);
...
const decryptJson = (data, key) => decrypt(data, key).then(
  (decryptedValue) => readAny(createDecoder$1(new Uint8Array(decryptedValue)))
);
const log$1 = createModuleLogger("y-webrtc");
...
const checkIsSynced = (room) => {
  ...
  if (!synced && room.synced || synced && !room.synced) {
    ...
    log$1("synced ", BOLD, room.name, UNBOLD, " with all peers");
  }
};
const readMessage$1 = (room, buf, syncedCallback) => {
  const decoder = createDecoder$1(buf);
  const encoder = createEncoder$1();
  const messageType = readVarUint$1(decoder);
  if (room === void 0) {
    return null;
  ...
  switch (messageType) {
    case messageSync$1: {
      writeVarUint$1(encoder, messageSync$1);
    ...
    case messageQueryAwareness$1:
      writeVarUint$1(encoder, messageAwareness$1);
      writeVarUint8Array$1(encoder, encodeAwarenessUpdate(awareness, Array.from(awareness.getStates().keys())));
      sendReply = true;
      break;
    case messageAwareness$1:
      applyAwarenessUpdate(awareness, readVarUint8Array$1(decoder), room);
      break;
...
const readPeerMessage = (peerConn, buf) => {
  ...
  log$1("received message from ", BOLD, peerConn.remotePeerId, GREY, " (", room.name, ")", UNBOLD, UNCOLOR);
  return readMessage$1(room, buf, () => {
    peerConn.synced = true;
    log$1("synced ", BOLD, room.name, UNBOLD, " with ", BOLD, peerConn.remotePeerId);
    checkIsSynced(room);
  });
};
const sendWebrtcConn = (webrtcConn, encoder) => {
  log$1("send message to ", BOLD, webrtcConn.remotePeerId, UNBOLD, GREY, " (", webrtcConn.room.name, ")", UNCOLOR);
  try {
    webrtcConn.peer.send(toUint8Array$1(encoder));
  } catch (e) {
  }
};
const broadcastWebrtcConn = (room, m) => {
  log$1("broadcast message in ", BOLD, room.name, UNBOLD);
...
class WebrtcConn {
  ...
  constructor(signalingConn, initiator, remotePeerId, room) {
    log$1("establishing connection to ", BOLD, remotePeerId);
    this.room = room;
    ...
    this.peer.on("connect", () => {
      log$1("connected to ", BOLD, remotePeerId);
      this.connected = true;
      ...
      if (awarenessStates.size > 0) {
        const encoder2 = createEncoder$1();
        writeVarUint$1(encoder2, messageAwareness$1);
        writeVarUint8Array$1(encoder2, encodeAwarenessUpdate(awareness, Array.from(awarenessStates.keys())));
    ...
    this.peer.on("close", () => {
      ...
      checkIsSynced(room);
      this.peer.destroy();
      log$1("closed connection to ", BOLD, remotePeerId);
      announceSignalingInfo(room);
    });
    this.peer.on("error", (err) => {
      log$1("Error in connection to ", BOLD, remotePeerId, ": ", err);
      announceSignalingInfo(room);
    });
...
const broadcastBcPeerId = (room) => {
  if (room.provider.filterBcConns) {
    const encoderPeerIdBc = createEncoder$1();
    writeVarUint$1(encoderPeerIdBc, messageBcPeerId);
    writeUint8(encoderPeerIdBc, 1);
    writeVarString(encoderPeerIdBc, room.peerId);
    broadcastBcMessage(room, toUint8Array$1(encoderPeerIdBc));
  }
};
let Room$1 = class Room {
  /**
   * @param {Y.Doc} doc
  ...
  constructor(doc, provider, name, key) {
    ...
    this._bcSubscriber = (data) => decrypt(new Uint8Array(data), key).then(
      (m) => this.mux(() => {
        ...
        if (reply) {
          broadcastBcMessage(this, toUint8Array$1(reply));
        }
      })
    );
    this._docUpdateHandler = (update, _origin) => {
      ...
      broadcastRoomMessage(this, toUint8Array$1(encoder));
    };
    this._awarenessUpdateHandler = ({ added, updated, removed }, _origin) => {
      const changedClients = added.concat(updated).concat(removed);
      const encoderAwareness = createEncoder$1();
      writeVarUint$1(encoderAwareness, messageAwareness$1);
      writeVarUint8Array$1(encoderAwareness, encodeAwarenessUpdate(this.awareness, changedClients));
      broadcastRoomMessage(this, toUint8Array$1(encoderAwareness));
    };
  ...
  connect() {
    ...
    const encoderSync = createEncoder$1();
    writeVarUint$1(encoderSync, messageSync$1);
    writeSyncStep1(encoderSync, this.doc);
    broadcastBcMessage(this, toUint8Array$1(encoderSync));
    const encoderState = createEncoder$1();
    writeVarUint$1(encoderState, messageSync$1);
    writeSyncStep2(encoderState, this.doc);
    broadcastBcMessage(this, toUint8Array$1(encoderState));
    const encoderAwarenessQuery = createEncoder$1();
    writeVarUint$1(encoderAwarenessQuery, messageQueryAwareness$1);
    broadcastBcMessage(this, toUint8Array$1(encoderAwarenessQuery));
    const encoderAwarenessState = createEncoder$1();
    writeVarUint$1(encoderAwarenessState, messageAwareness$1);
    writeVarUint8Array$1(encoderAwarenessState, encodeAwarenessUpdate(this.awareness, [this.doc.clientID]));
    broadcastBcMessage(this, toUint8Array$1(encoderAwarenessState));
  }
  disconnect() {
    signalingConns.forEach((conn) => {
      if (conn.connected) {
    ...
    broadcastBcMessage(this, toUint8Array$1(encoderPeerIdBc));
...
const openRoom = (doc, provider, name, key) => {
  if (rooms.has(name)) {
    throw create$4(`A Yjs Doc connected to room "${name}" already exists!`);
  }
  const room = new Room$1(doc, provider, name, key);
  rooms.set(
    name,
    /** @type {Room} */
    room
...
const publishSignalingMessage = (conn, room, data) => {
  if (room.key) {
    encryptJson(data, room.key).then((data2) => {
      conn.send({ type: "publish", topic: room.name, data: toBase64$1(data2) });
    });
  } else {
...
class SignalingConn extends WebsocketClient {
  constructor(url) {
            ...
            switch (data.type) {
              ...
              case "signal":
                if (data.signal.type === "offer") {
                  ...
                  if (existingConn) {
                    ...
                    if (localToken && localToken > remoteToken) {
                      log$1("offer rejected: ", data.from);
                      return;
                    }
                ...
                if (data.signal.type === "answer") {
                  log$1("offer answered by: ", data.from);
                  const existingConn = webrtcConns.get(data.from);
                  existingConn.glareToken = void 0;
                }
                if (data.to === peerId) {
    ...
    this.on("disconnect", () => log$1(`disconnect (${url})`));
  }
}
...
const wait$1 = (ms) => new Promise((resolve2) => setTimeout(resolve2, ms));
...
class Room2 extends TypedEventEmitter {
  constructor({
    db,
    indexedDbProvider,
    webRtcProvider,
...
const rtop = (request) => create$1((resolve2, reject2) => {
  request.onerror = (event) => reject2(new Error(event.target.error));
  request.onsuccess = (event) => resolve2(event.target.result);
});
const openDB = (name, initDB) => create$1((resolve2, reject2) => {
  const request = indexedDB.open(name);
  request.onupgradeneeded = (event) => initDB(event.target.result);
  request.onerror = (event) => reject2(create$4(event.target.error));
...
const iterateOnRequest = (request, f) => create$1((resolve2, reject2) => {
...
class IndexeddbPersistence extends Observable$1 {
  ...
  constructor(name, doc) {
    ...
    this.whenSynced = create$1((resolve2) => this.on("synced", () => resolve2(this)));
    this._db.then((db) => {
      this.db = db;
...
var _localStorage = new VarStoragePolyfill2();
...
var LocalStoragePolyfill2 = class {
  /**
   * @param {string} room
   */
  constructor(room) {
...
var BC = typeof BroadcastChannel === "undefined" ? LocalStoragePolyfill2 : BroadcastChannel;
...
const loadRoom = (db) => async (serverRoom) => {
  ...
  const room = db.collections[collectionKey][roomId] ?? new Room2({ db, ...serverRoom });
...
const syncRegistry = (db) => (
  ...
  async () => {
    ...
    const { rooms: rooms2, token } = syncResult ?? {};
    if (token && typeof token === "string") {
    ...
    if (rooms2 && typeof rooms2 === "object" && Array.isArray(rooms2) && rooms2.length >= 2) {
      db.debug("setting new rooms", rooms2);
      setLocalRegistry(db)(rooms2);
      db.registry = rooms2;
    } else {
      return false;
    }
...
const loadRooms = (db) => async (rooms2) => {
  const loadedRooms = [];
  db.debug("loading rooms", rooms2);
