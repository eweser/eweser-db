...
async function loadYSweet(db, room, withAwareness = true, awaitConnection = false, maxWait = 30000) {
    ...
    async function pollForYSweetConnectionAndAwait(maxWait) {
        let waited = 0;
        while (room.ySweetProvider?.status !== 'connected') {
            await (0, shared_1.wait)(300);
            waited += 300;
            if (waited >= maxWait) {
                db.debug('timed out waiting for ySweet connection', room.name, waited, maxWait, room.ySweetProvider);
