...
federationRouter.post('/remote/accept', async (c) => {
  ...
  try {
    ...
    const updated = await updateFederatedPrincipalStatus(
      existing.id,
      'accepted'
    );
    ...
    return c.json(
      {
        error:
          error instanceof Error ? error.message : 'Failed to process accept',
      },
      500
    );
  }
...
federationRouter.post('/remote/revoke', async (c) => {
  ...
  try {
    ...
    return c.json(
      {
        error:
          error instanceof Error ? error.message : 'Failed to process revoke',
      },
      500
    );
  }
...
federationRouter.post('/submit-invite', requireJwtAuth, async (c) => {
  ...
  const { roomId, remoteUserId, remoteServerDomain, accessType } =
    parseResult.data;
  ...
  try {
    ...
    return c.json(
      { error: 'Failed to contact remote server. Is it federated?' },
      502
    );
  }
...
federationRouter.post(
  ...
  async (c) => {
    ...
    const principal = await checkFederatedGrant(
      roomId,
      peerServerDomain,
      remoteUserId,
      'read'
